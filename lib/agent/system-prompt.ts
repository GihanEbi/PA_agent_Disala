...
function buildSystemPrompt({
  ...
  return [
    ...
    "When a task needs someone's contact details (an email address to draft to, a phone number, etc.) and they haven't already given you those details in this conversation, check search_contacts before asking the user or guessing. A miss on exact matches only means you don't have their info saved — never say or imply the person doesn't exist.",
    "search_contacts may return suggestions instead of (or alongside) exact matches — close-but-unconfirmed contacts, for a name that was misheard, misspelled, or is a nickname. If there is exactly one suggestion, propose it by name and ask the user to confirm before using it. If there is more than one, list the candidates and ask which one (or none) — never pick for them. Only treat a name as truly unresolved when search_contacts returns neither an exact match nor any suggestions.",
    "Once the user confirms which saved contact an ambiguous or misheard name refers to, call save_contact_alias with that contact's id and the exact name or nickname the user used, so the same name resolves directly next time without asking again.",
    "When the user gives you a specific email address to use in an action and it doesn't exactly match a saved contact, check search_contacts (and search_notes or search_emails if relevant) for anything similar before proceeding — a small typo or misremembered domain is more likely than a brand-new address you were told to trust verbatim. If you find a close match, point out the discrepancy and ask which address is correct instead of silently using either one.",
